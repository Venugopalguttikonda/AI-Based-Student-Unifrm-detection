...
# Define base paths
BASE_DIR = os.getcwd()  # This sets the base directory to your current folder
...
for directory in [FACE_DATA_DIR, MODEL_DIR, VIOLATION_DIR, VERIFIED_DIR, UNAUTHORIZED_DIR]:
...
if not all(os.path.exists(path) for path in [yolo_model_path, sp_path, face_rec_path]):
    raise FileNotFoundError("Error: Required models not found in the 'models' directory.")
...
def send_email(to_email, subject, message, image_path=None):
    """Sends an email notification with an optional image attachment."""
    try:
        msg = EmailMessage()
        msg["Subject"] = subject
...
def capture_student_image():
    ...
    time.sleep(3)  # Delay for alignment
...
def recognize_student(image):
    """Identifies a student using facial recognition."""
...
def process_detection(frame, image_path):
    ...
    if student_name and student_email:
        ...
        if non_uniform_detected:
            print(f"Alert: {student_name} detected without a uniform. Sending fine email.")
            ...
            message = (f"Dear {student_name},\n\n"
                       ...
                       "Please ensure compliance within 3 days.\n\n"
                       "Best regards,\nAdministration")
        ...
        print("Unknown person detected. Checking uniform status...")

        if non_uniform_detected:
            print("Unknown person is in without a uniform. Marking as unauthorized entry.")
        elif uniform_detected:
            print("Unknown person is in uniform, but identity is not recognized.")
        else:
            print("Unknown person detected, but unable to determine uniform status.")
